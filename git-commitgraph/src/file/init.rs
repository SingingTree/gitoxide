...
fn try_get_file(file_data: FileData) -> Result<File, Error> {
    let data: &[u8] = match file_data {
        FileData::FileBacked(ref data, _) => data,
        FileData::InMemory(ref data) => data,
    };
...
impl<'a> TryFrom<Box<[u8]>> for File {
    // TODO(bryce) revert lifetimes and investigate further
    type Error = Error;

    fn try_from(data: Box<[u8]>) -> Result<Self, Self::Error> {
        try_get_file(FileData::InMemory(data))
    }
}
...
impl TryFrom<&Path> for File {
    ...
    fn try_from(path: &Path) -> Result<Self, Self::Error> {
        ...
        try_get_file(FileData::FileBacked(data, path.to_path_buf()))
    }
}
